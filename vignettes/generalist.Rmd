---
title: "5. Generalist plots across models"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{5. Generalist plots across models}
...
The generalist plotting functions read the standardised membership schema and
so work on **any** fitted model via a `model = c("dmm", "lda", "hdp")`
argument. This lets you render the same view for each model and compare like
with like.
...
Colour a Bray-Curtis PCoA by any model's dominant group, or by a metadata
variable:
...
Faceted by a model's dominant group, annotated with assignment confidence:
...
Near-binary for DMM, gradient for LDA/HDP — the visual signature of hard vs
...
Each model's groups against SymPortal profiles (rows grouped by clade); tile
colour is mean confidence, number is sample count:
