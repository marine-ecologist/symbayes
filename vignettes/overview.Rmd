---
title: "1. SymBayes Overview"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{1. SymBayes Overview}
...
SymPortal assigns ITS2 amplicon data to *type profiles* using deterministic
co-occurrence rules. This has two well-known failure modes: it **over-splits**
(creating many near-identical profiles that differ only in minor intragenomic
variants), and it **mistakes mixed symbiont communities for unique types**
(giving an admixed sample a single compound profile name).

`symbayes` reframes profiling as probabilistic mixture modelling and provides
three complementary models, each answering a different question:
...
Everything flows through a single object created by the `import()` function which 
imports sections of the Symportal output:
...
`sp$dmm`, `sp$lda`, or `sp$hdp`, all sharing one schema, so
the generalist plots work on any of them.
...
**Generalist** functions take a `model=` argument and work on any fitted model:
...
**Specialist** functions are prefixed by model and cover diagnostics unique to
that model. See the per-model vignettes:

- `vignette("dmm")` — model selection, prediction, LOO, simulation
- `vignette("lda")` — K search, topic similarity, topic summary
- `vignette("hdp")` — inferred component number, over-splitting quantification
