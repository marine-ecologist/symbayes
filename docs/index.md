...
Threshold-free, mixture-aware profiling of Symbiodiniaceae ITS2 amplicon
data — a probabilistic alternative deterministic type profiles.
`symbayes` reframes profiling as probabilistic mixture modelling to
determine intragenomic variants and admixtures using  
Dirichlet-Multinomial Mixture, Latent Dirichlet Allocation, and
...
- **Entry points**: `import`, `filter_samples`, `fit_dmm`, `fit_lda`,
  `fit_hdp`
- **Generalist plots** (`model=` argument): `plot_pcoa`, `plot_barplot`,
  `plot_exemplars`, `plot_membership`, `plot_contingency`,
  `plot_top_seqs`
